...
void AGeometryHubActor::DoActorSpawn1()
{
	...
	if (World)
	{
		...
		{
			const FTransform GeometryTransform = FTransform(FRotator::ZeroRotator, FVector(0.0f, 120.0f * i, 100.0f));
...
void AGeometryHubActor::DoActorSpawn2()
{
	...
	if (World)
	{
		for (int32 i = 0; i < 10; i++)
		{
			const FTransform GeometryTransform = FTransform(FRotator::ZeroRotator, FVector(0.0f, 120.0f * i, 300.0f));
			ABaseGeometryActor* Geometry = World->SpawnActorDeferred<ABaseGeometryActor>(GeometryClass, GeometryTransform);// spawn Actora с отложенным вызовом BeginPlay()
			...
			if (Geometry)
			{
				...
				Geometry->SetGeometryData(Data);
				Geometry->FinishSpawning(GeometryTransform);// вызов BeginPlay() в ручную
			}
		}
	}
}
